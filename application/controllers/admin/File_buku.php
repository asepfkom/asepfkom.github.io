...
class File_buku extends CI_Controller {
	...
	public function kelola($id_buku)
	{
		...
		//end validation
		$data = array(
			'title' 	 => 'Data File_buku :'.$buku->judul_buku.'('.count($file_buku).')',
			'file_buku'  => $file_buku,
			'buku'		 => $buku,
	...
	public function edit($id_file_buku)
	{
		...
		if($valid->run()) {
			...
			$config['upload_path'] 	 = './assets/upload/files/';
			$config['allowed_types'] = 'pdf|doc|docs|xls|xlsx|ppt|pptx';
			$config['max_size']  	 = '12000';//kb
			$this->upload->initialize($config);
			if ( ! $this->upload->do_upload('nama_file')){
		//end validation
		...
		//end masuk database
		$data = array(
			'title' 	 => 'Edit File_buku :'.$buku->judul_buku.'('.count($file_buku).')',
			'file_buku'  => $file_buku,
			'buku'		 => $buku,
			'isi' 		 => 'admin/file_buku/edit');
		$this->load->view('admin/layout/wrapper',$data, FALSE);
